...
interface SharedIntentPayload {
  urls: string[];
  /**
   * Android-only. Distinguishes "Open with Lumen" (`VIEW` — the user
   * tapped a file in their file browser and chose Lumen) from "Send to
   * Lumen" (`SEND` / `SEND_MULTIPLE` — share-sheet capture). We forward
 ...
 *   - `shared-intent` plugin   — Android "Share to Lumen" intent
...
export function useAppUrlIngress() {
  ...
  useEffect(() => {
    ...
    // side, so any "Open with Lumen" intent that arrived AFTER cold
