...
describe('parseOAuthCallbackUrl', () => {
  it('should extract error params from a failed deeplink callback', () => {
    ...
    const url =
      'lumen://auth-callback?error=server_error&error_code=unexpected_failure&error_description=Unable+to+exchange+external+code%3A+c578' +
  ...
  it('should extract tokens from a successful implicit-flow callback', () => {
    const url =
      'lumen://auth-callback#access_token=abc123&refresh_token=def456&type=magiclink&next=/user';
  ...
  it('should extract error params when present only in the query string', () => {
    const url =
      'lumen://auth-callback?error=access_denied&error_code=denied&error_description=nope';
  ...
  it('should return all-null params for a callback with no data', () => {
    const params = parseOAuthCallbackUrl('lumen://auth-callback');
