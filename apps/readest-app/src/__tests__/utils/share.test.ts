...
describe('parseShareDeepLink', () => {
  ...
  it('parses lumen://share/{token}', () => {
    expect(parseShareDeepLink(`lumen://share/${VALID_TOKEN}`)).toEqual({ token: VALID_TOKEN });
  });
  ...
  it('rejects tokens of the wrong length', () => {
    expect(parseShareDeepLink('lumen://share/short')).toBeNull();
    expect(parseShareDeepLink(`lumen://share/${VALID_TOKEN}extra`)).toBeNull();
  });
  ...
  it('rejects tokens with disallowed characters', () => {
    ...
    expect(parseShareDeepLink(`lumen://share/${bad}`)).toBeNull();
  });
  ...
  it('rejects lumen:// URLs whose host is not "share"', () => {
    expect(parseShareDeepLink(`lumen://book/${VALID_TOKEN}`)).toBeNull();
    expect(parseShareDeepLink(`lumen://annotation/${VALID_TOKEN}`)).toBeNull();
  });
